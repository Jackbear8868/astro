...
product against the run stored where that pointing's config sends it: arrays
value by value with NaNs in the same places, FITS plane by plane, npz field by
field, JSON key by key, everything else byte for byte. `meta.json`'s `created`
and `git_commit`, and paths that follow the output directory, are normalised
away; logs are compared for presence but not for content.
...
A change that is meant to keep the answer identical -- a refactor, a rename, a
faster loop -- has to pass this. p05 and p14 between them drive both branches of
the sky region: p05 restricts by a range, p14 excludes a box.

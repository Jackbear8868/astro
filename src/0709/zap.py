...
zap — 用 ZAP 扣天空。指定「ZAP 對象 target」「用哪個 cube 建的 mask（mask_from）」「mask 方法」。
  ...
  跑法： conda run -n astro python src/0709/zap.py --target wsky --mask-from nosky --ncpu 16
         conda run -n astro python src/0709/zap.py --target wsky --mask-from nosky --mask-method claude --ncpu 16
...
def zap(target, mask_from, mask_method="sep", ncpu=16):
    ...
    print(f"[zap] {label} 完成，{time.time()-t0:.0f}s -> {settings.zap_path(target, mask_from, mask_method)}", flush=True)

if __name__ == "__main__":
    argv = sys.argv[1:]
    target = mask_from = None
    mask_method, ncpu = "sep", 16
    i = 0
    while i < len(argv):
        if argv[i] == "--target":
            target = argv[i + 1]; i += 2

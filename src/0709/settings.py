"""
src/0709 pipeline 的【唯一設定檔】：所有路徑 + 物理參數只在這裡定義一次。
zap.py / mask.py / eval_spectrum.py 全部 import 這裡；改參數只改這一份，不會分岔。

※ 所有相對路徑都相對「專案根目錄」，請一律從根目錄執行，例如：
    conda run -n astro python src/0709/zap.py wsky nosky --ncpu 16
"""
...
# --- zap 產物：依「ZAP 對象 × mask(方法 × 來源)」命名，每個實驗一個資料夾 ---
...
def run_dir(target, mask_from, mask_method="sep"):
    return CUBES_DIR / f"{target}_maskfrom-{_run_suffix(mask_from, mask_method)}"  # sep: wsky_maskfrom-nosky；其他: wsky_maskfrom-<method>-nosky

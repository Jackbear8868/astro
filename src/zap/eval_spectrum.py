...
eval_spectrum — 任選一塊空間區域，取平均(mean/median) sky-subtracted spectrum、畫圖、
  ...
  --y-range 固定所有圖的 y 軸範圍，--compare-raw 疊上未扣天空的 raw cube 一起比較，
  跑完存 png + spectra_report.json 到 --out-dir，並印到終端機）：
    conda run -n astro python src/zap/eval_spectrum.py \\
        ...
        --mask-from NEnosky --mask-method seg1sigma_brq --y-range -20 20 --compare-raw \\
  ...
  import 用法範例：
    ...
    # compare_raw=True 時每張圖多疊一條 raw(未扣天空) spectrum，stats 多出 raw_mean/raw_sigma/… 這組 key：
    report = es.spectra_report(fits_path, "NEnosky", regions, out_dir="...", compare_raw=True)
...
# 兩條疊圖 spectrum 的固定配色：主(subtracted)藍、raw(未扣天空)橘——對比高又色盲友善；
# 線條半透明（下面 plot_spectrum 用 alpha）讓兩條互不完全遮住，交疊處也看得到對方。
SUBTRACTED_COLOR = "#1f77b4"
RAW_COLOR        = "#e8710a"
...
# ======================= 畫圖 =======================
def plot_spectrum(wavelengths, spectrum, out_path, label=None, title=None, wl_range=None, y_range=None, overlay=None):
    """單條 flux vs wavelength 曲線，0 基準線，右側顯示 spectrum_stats 摘要，存檔。
       ...
       overlay=(spectrum2, label2) 時再疊第二條線（虛線、灰色），右側同時列兩組 stats——
       用來把 sky-subtracted 與 raw(未扣天空) 疊在同一張圖直接比較（扣得乾不乾淨 + 源有沒有被吃掉）。"""
    ...
    ax.plot(wavelengths, spectrum, lw=0.9, alpha=0.85, color=SUBTRACTED_COLOR, label=label)
    if overlay is not None:
        overlay_spec, overlay_label = overlay
        ax.plot(wavelengths, overlay_spec, lw=0.9, ls="--", alpha=0.7, color=RAW_COLOR, label=overlay_label)
    ...
    if label or overlay is not None: ax.legend(loc="upper left", fontsize=8)
    ...
    if overlay is None:
        ...
        # 兩塊 stats 各自套對應線條顏色，於中線上下貼著排（不再頂天立地分很開），跟圖上兩條線一眼對得起來。
        blocks = [(label or "subtracted", stats, SUBTRACTED_COLOR, 0.53, "bottom"),
                  (overlay[1] or "raw", spectrum_stats(overlay[0], wavelengths, wl_range), RAW_COLOR, 0.47, "top")]
        key_width = max(len(k) for _, s, *_ in blocks for k in s)
        for name, s, color, y, va in blocks:
            txt = "\n".join([f"[{name}]"] + [f"{k:<{key_width}} = {v:.4g}" for k, v in s.items()])
            stat_ax.text(0.0, y, txt, fontsize=8, va=va, ha="left", family="monospace",
                         color=color, transform=stat_ax.transAxes)
...
def spectra_report(fits_path, mask_from, regions, out_dir, bands=WAVELENGTH_BANDS, statistic="mean", y_range=None, compare_raw=None):
    """regions = {region_name: 2D bool region array}。每個 region 只抽一次 spectrum（不必為
       ...
       y_range 可給 (ymin,ymax) 套用到所有圖；或給 dict {region_name: (ymin,ymax) 或 None} 逐 region
       指定（缺的 region → None → 自動縮放）。例：whole/blank 固定 -20~20、box 自動縮放。不給則全部自動。

       compare_raw：把 raw(未扣天空) cube 的同一個 region spectrum 疊在同一張圖比較。可給——
         None / False → 不比較（維持原本單線行為）。
         True         → 用 settings.input_cube_path(mask_from) 當 raw；NE 實驗即 esonosky（ESO
                        已扣天空、還沒過 ZAP 的 nosky baseline），拿來看 ZAP 在它之上多動了什麼。
         cube 名 str  → settings.input_cube_path(該 cube)，自己指定要比哪顆 raw。
       raw 的 stats 以 raw_<key>（raw_mean/raw_sigma/…）附加在原 stats 旁邊，不改動原 JSON 結構。
       回傳 {region_name: {band_name: stats}}，圖存到 out_dir/spec_<region_name>_<band_name>.png。"""
    out_dir = Path(out_dir); out_dir.mkdir(parents=True, exist_ok=True)
    wl = wavelength_axis(mask_from)
    run_name = Path(fits_path).parent.name    # 實驗名（run 資料夾名），加在每張圖標題最前面

    raw_cube = raw_path = None
    if compare_raw:
        raw_cube = mask_from if compare_raw is True else compare_raw
        raw_path = settings.input_cube_path(raw_cube)
    ...
    for region_name, region in regions.items():
        ...
        if raw_path is not None:
            raw_mean, raw_median = extract_spectrum(str(raw_path), region)
            raw_spectrum = raw_mean if statistic == "mean" else raw_median
        region_y_range = y_range.get(region_name) if isinstance(y_range, dict) else y_range
        report[region_name] = {}
        for band_name, wl_range in bands.items():
            out_path = out_dir / f"spec_{region_name}_{band_name}.png"
            overlay = (raw_spectrum, f"raw {raw_cube} ({statistic})") if raw_spectrum is not None else None
            plot_spectrum(wl, spectrum, out_path, label=f"{region_name} ({statistic})",
                          title=f"{run_name}  |  {region_name}  [{band_name}Å]", wl_range=wl_range, y_range=region_y_range, overlay=overlay)
            stats = spectrum_stats(spectrum, wl, wl_range)
            if raw_spectrum is not None:
                stats.update({f"raw_{k}": v for k, v in spectrum_stats(raw_spectrum, wl, wl_range).items()})
            report[region_name][band_name] = stats
    return report
...
if __name__ == "__main__":
    ...
    usage = ("用法: eval_spectrum.py --fits-path PATH --mask-from CUBE [--mask-method M] [--out-dir DIR] "
              "[--statistic mean|median] [--y-range YMIN YMAX] [--compare-raw [CUBE]] [--box NAME Y0 Y1 X0 X1 ...]\n"
              "  永遠包含 whole/blank 兩個 region；--box 可重複給多個自訂 x,y 範圍。\n"
              "  --y-range 固定 whole/blank 的 y 軸範圍（box 訊號很小、一律自動縮放；不給則全部自動縮放）。\n"
              "  --compare-raw 疊上 raw(未扣天空) cube 的同區域 spectrum 一起比較；不帶 CUBE 則用 mask-from 的 raw"
              "（NE 實驗即 esonosky nosky baseline），帶 CUBE 名則指定要比哪顆。")
    ...
    while i < len(argv):
        ...
        elif argv[i] == "--compare-raw":
            if i + 1 < len(argv) and not argv[i + 1].startswith("--"):   # 可選帶 cube 名
                compare_raw = argv[i + 1]; i += 2
            else:                                                        # 不帶 → 用 mask_from 的 raw
                compare_raw = True; i += 1
        elif argv[i] == "--box":
    ...
    y_range_map = {"whole": y_range, "blank": y_range}   # whole/blank 吃 --y-range；box 一律自動縮放（訊號很小）
    for name, y_box_range, x_range in boxes:
        regions[name] = region_box(mask_from, y_box_range, x_range)
        y_range_map[name] = None

    report = spectra_report(fits_path, mask_from, regions, out_dir, statistic=statistic, y_range=y_range_map, compare_raw=compare_raw)

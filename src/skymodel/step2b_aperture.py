...
為什麼換
    step2 用的是 SExtractor 切出來的不規則形狀,大小從 3 到 13,782 spaxel,
    而且形狀完全由偵測門檻決定 —— 門檻動一點,抽出來的光譜就變。圓形孔徑是
    外加的、固定的:同一個半徑套在每個源上,和偵測門檻無關,可重現。
    (教授 2026-08-06 指示:the data should be the mean spectrum within an aperture)

半徑 = 6 px = 1.2 arcsec
    PSF 隨波長變窄(實測 FWHM 4.29 px @5500 A → 3.53 px @9200 A,窄 18%),
    所以固定孔徑在不同波長收到的流量比例不同,會造成假的連續譜斜率:

        r = 3 px (S/N 最佳)   假斜率 16.5%    ← 對連續譜分類是致命的
        r = 4 px              假斜率  6.7%
        r = 6 px              假斜率  0.4%    ← 採用
        r = 8 px              假斜率  0.01%   但 S/N 只剩 51%

    6 px 是「假斜率壓到 0.4%」和「S/N 還有 67%」的折衷。

圓心 = 整數像素
    做法:背景扣除後的流量加權形心 → 四捨五入 → 用整數圓心重畫圓、再算一次
    形心 → 直到整數圓心不再移動。

    ① 背景一定要扣。白光背景 0.613,而多數源的淨白光只有 0.3–1.0 —— 底座和
       源一樣大,不扣的話形心量到的有一半是遮罩形狀。實測暗源與邊緣源會偏
       0.7–1.9 px。
    ② 迭代要在圓內做,不能只在 seg 遮罩內。只做後者的話圓心仍然繼承 seg 的
       形狀,而那正是我們想擺脫的東西。
    ③ 整數圓心的代價是最多 0.71 px 的捨入誤差 = r=6 時約 0.1–0.2% 的流量;
       換到的是每個源完全相同的孔徑形狀與像素數(113 px),源之間直接可比。
...
def sum_spectra_by_mask(cube_path, masks, chunk=200):
    ...
    不能用 step2 的標籤圖版本 —— 相鄰的源(ID 20/21/22、36/11/32)孔徑會重疊,
    標籤圖逼我們把重疊的像素判給其中一個,等於偷偷截掉另一個。

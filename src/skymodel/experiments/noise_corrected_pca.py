...
正好就是 basis 集中的地方(K=30 有 81% 的能量落在 100 個通道上)。

實測跨 spaxel 的散布:sigma_emp / sigma_stat = 1.323
    var_emp   = 1.75 · var_stat
    var_訊號  = 0.75 · var_stat
    → 天光線通道上,SVD 看到的變異數有 57% 是光子雜訊
...
alpha 是「STAT 佔真實雜訊的比例」。STAT 已知低估(線外量到約 1.37 倍 sigma),
所以不硬性設 alpha=1,而是掃描讓留出資料決定 —— alpha=0 就是現行做法。
...
def main():
    ...
    # flux 指標(未加權)與 chi 指標(除以雜訊)一起報 —— 先前三個「考慮雜訊」的
    # 改動都只量了 flux 就被否決,但 chi2 加權擬合的經驗顯示兩個指標會往相反
    # 方向走。只量一個等於預設了答案。

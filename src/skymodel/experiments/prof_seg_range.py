...
學天光的範圍必須排除星系:遮罩外面若還有星系的延展暈,不多排除一圈,它就會被當成
天空學進 basis 裡。換一套遮罩就要重問一次「遮罩外面還有星系嗎、要往外推多遠」,這
支就量這件事。做法是沿「離主源遮罩的距離 d」把 blank 切成同心環,每環取 pseudo-r
的中位數:

    profile(d) = median{ pseudo_r(p) : p 是 blank, 離主源 d 到 d+bin }

遠處的環當基線,星系的暈會讓近處的環高出基線,超出量掉進雜訊裡的那個 d 就是遮罩外
面還受污染的範圍。門檻不能用每環中位數的統計誤差 sigma/sqrt(N):每環格數很多,那
個誤差極小,等於在問 profile 是不是數學上完全平坦,而背景本來就不平坦。門檻改用遠
場自己的環對環起伏,見 noise_floor_sys —— 量的是量得出來的污染,不是物理上為零。

那個乾淨起點由各顆自己的背景起伏決定,不能直接當通用規則。要選一個能套用到全部
pointing 的排除半徑,看的是最後那張取捨表:同一個 R 之下各顆還剩多少污染、多少
blank。小源的翼會混進來,所以另外要求離任何源 > r_other。
...
def noise_floor_sys(rows, far):
    ...
    每環的格數很多,統計誤差極小,拿它當門檻等於在問 profile 是不是數學上完全平坦,
    答案永遠是到視野邊緣都還不乾淨。正確的底是遠場自己的環對環起伏:那裡沒有星系,
    剩下的散布就是背景系統性起伏的大小,近處的超出量小於它就代表量不出來了。
    """
    m = rows[:, 0] > far
    if m.sum() < 3:
...
def first_clean(rows, floor, nsigma):
    ...
    從近往遠找第一個乾淨的環會被 profile 的起伏騙到:它可能穿過一次基線又升回去。
    要的是之後全部都乾淨,所以從遠端往回掃。
    """
...
def main():
    ...
    for k, n in enumerate(args.n):
        ...
        # 取捨表:排除半徑 R 換到的殘留污染與剩下的 blank。只報一個乾淨起點不夠,
        # 它由每顆自己的背景起伏決定;通用規則要看同一個 R 之下各顆的污染量。

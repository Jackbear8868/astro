...
任何「拿鄰近訓練點做加權平均」的補法都只能在有樣本的地方作用,被源遮住的大洞裡無
值可用;a(y) 伸得進洞裡是因為它是被整列共用的一個參數。所以方向是讓剩下的結構也
變成參數共用的模型:

    s(y, x)  ≈  Sum_{k=1..R}  u_k(y) · v_k(x)

u_k 由整列決定、v_k 由整行決定,自動伸得進洞;現行的 mu + a(y) + b(x) 是它的特例
(加法模型 = 秩 3),所以這是嚴格的推廣。判準在跑之前訂好:

    ① 配對勝率 > 55%      同一格比同一格,真值的雜訊被配對消掉,比比散布敏感
    ② 扣掉雜訊底後誤差更小  sqrt(scatter² − sigma_n²);完美的預測器也只到 sigma_n
...
def lowrank_complete(s, obs, rank, init, n_iter=60, tol=1e-7):
    ...
    init 用 mu + a(y) + b(x):觀測很稀疏的 row/col 會讓 SVD 被少數幾格帶著走,
    一個已經合理的曲面當起點可以把它們釘住。
    """
...
def main():
    ...
    for hi, (hole, ctr) in enumerate(hs):
        ...
        truth = s[hole]
        # 現行的場就是 M:build_amplitude_field 回傳的正是 median_polish 的結果
        cur   = M[hole]
        preds = {CUR: cur}
        for R in args.ranks:
    ...
    # |偏差| 也要報:散布和配對勝率被雜訊主導,而影響 over-subtraction 的是偏差 ——
    # 場在源旁系統性偏高保留率就掉,散布卻會在環上平均掉。

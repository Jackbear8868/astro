...
blank 是用白光影像上的 SExtractor 偵測界定的,而只有發射線、沒有連續譜的電離氣體暈
對寬波段白光幾乎沒有貢獻,可能整片落在 blank 裡。真的落進去的話,SVD 會學到一條帶
紅移發射線的成分再從源身上減掉,而殘差類的指標看不到:源被扣掉殘差反而更小。判準
是那些波長上 basis 有沒有超出局部散布的結構,對照組取既沒有天光線也沒有星系線的波
長,沒有污染的話兩者分布一樣。
...
def line_strength(B, c):
    ...
    線心取 ±CORE 的和,背景取兩側 GAP..BG 的中位數,再除以背景的散布,所以回傳值
    是幾個 sigma,不同 basis、不同波長之間可比。
    """
    nz = B.shape[1]
...
def main():
    ...
    IN = np.load(STEP03 / "continuum_iterations.npz")["line_mask"][0]
    B  = np.load(STEP03 / f"sky_line_basis_{args.basis}_K{args.K}.npy")
    B  = B / np.linalg.norm(B, axis=1, keepdims=True)
    nz = wl.size

    sf = STEP04 / "source_fits.npz"
    if not sf.exists():
        raise SystemExit(f"{sf} 不存在,先跑這顆 pointing 的 pipeline:\n"

...
量到的問題:blank 樣本裡有 4,470 個 spaxel(5.3%)含 Haro 11 電離氣體暈的
發射線(S/N>3)。暈只有發射線、沒有連續譜,所以白光影像上的 SExtractor 偵測
(教授指定)看不到它。於是 SVD 學到一條含紅移 [O III]/Ha 的成分,再從真正的
源身上減掉 —— 端到端量到核心 [O III] 損失 4.2%、r>150 px 全部消失。

要區分兩件事:
    源遮罩(教授指定,白光 SExtractor)   界定「哪裡是源」,不動
    學 basis 的樣本純度                 界定「哪些 spaxel 乾淨到可以當天空的
                                        範本」,這是另一件事,而且只影響 step3

損失可以拆成兩塊(高對比成像文獻的標準拆法):
    一般性投影   源的光譜有一部分落在 K+1 維子空間裡    注入實驗量到 1.9%
                 (注入的假源不在訓練集裡,所以只有這塊)
    自我扣除     源自己在訓練集裡,連 basis 都被它污染   [O III] 4.2% − 1.9% ≈ 2.3%

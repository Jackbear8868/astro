...
s 場是 s_hat = mu + a(y) + b(x)。它抓得到整列整行共有的條紋,但抓不到只在局部出現
的結構,而被源遮住的大洞裡本來就沒有訓練點。各顆 pointing 的星系落在視場的不同位
置,所以只要殘差裡有一部分是所有 pointing 共有的固定圖案,就可以疊:一顆的洞在別
顆那裡是 blank,量得到。

但共有的圖案不一定是儀器。星系的延展暈在天上位置固定,在每一顆裡都落在同一個天空
座標,一樣會製造共有的圖案;把它疊進 s 場再拿去扣,就是把星系的暈當成天空扣掉。

    是儀器  ->  振幅和離星系的距離無關,結構是條紋
    是暈    ->  離星系越近越強,結構是徑向遞減

這支先跑這個判別,不過關就不要往下做。共同座標靠 CRPIX:各顆的 CRVAL 相同只差
CRPIX,所以天空位置就是 (i − CRPIX1, j − CRPIX2),整數位移就對得上,不必重取樣。

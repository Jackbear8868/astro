...
發現的問題:通道 151(4788.4 A)有 12 個 blank spaxel 的值是 −1,750 到 −548,029,
而正常範圍是 −73 到 +179。它們在空間上成團((238,270) 附近的 2x2、(262,81) 附近
的一條),是宇宙射線或壞畫素在重採樣時被抹開的形態。

兩層後果:
    mean_sky 用 np.nanmean,那個通道被拉到 1.8(鄰居 29.5)
    更嚴重:SVD 的訓練矩陣裡就有這些值,於是產生一條「100% 能量在通道 151」
    的 basis —— 它描述的不是天空,是那 12 個壞 spaxel。
    然後任何模板只要藍端覆蓋越過 4788 A,那條 basis 的係數就失去約束,
    chi2_all 暴增 285 倍,該模板在那個 z 之上被假性禁止。
...
def main():
    ...
    # R = D − C_sky 裡還含著天光線,天光線通道上每個 spaxel 的 R 都是 ~1600,
    # 直接除以散布會讓每個 spaxel 都「超過 20 sigma」。

...
線外已確認在雜訊底線上(lag3 差分估計 = 總 sigma),線內則是我們和 ESO 唯一
有差別的地方(真雜訊/STAT:我們 1.077、ESO 1.220)。所以要問的是:線內剩下的
誤差是平均分布在 1349 個通道上(那就是雜訊,沒得改),還是集中在少數幾條線
(那就可行動)。

逐通道對 62,475 個 blank spaxel 取兩個量,拆法不依賴 STAT:

    mean(c)   同調誤差。雜訊會被平均掉(sigma/sqrt(62475) ~ 0.03),所以顯著

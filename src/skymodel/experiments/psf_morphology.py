...
「一顆 G 星」和「一個 z≈0 的早型星系」在光譜上近乎簡併(實測:四條星系本徵
成分在 z=0 能把 G 星重建到 4% 殘差)。chi2 分不出來,形態分得出來。

用聚集度指數,不用 PSF 擬合:

    C = 半徑 2 px 內的流量 / 半徑 5 px 內的流量

理由 —— 這個場裡沒有已知的參考點源,所以任何需要「先知道 PSF」的方法都會
被異常值帶偏。前兩次嘗試就是這樣失敗的:
    高斯 + 自由背景  → 暗源的振幅/寬度/背景互相退化,量出 FWHM 18–33 px
    Moffat + 由最亮的源定 beta → 最亮的是 Haro 11(橫跨 14,040 spaxel 的
                                 大星系),beta 被壓到下界,PSF 基準變成 2.19 px

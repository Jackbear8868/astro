...
動機:天空的兩個成分在兩個區域各自可測,但現行做法把它們混在一起解。
    ...
    line1 外   連續譜主導流量,天光線 basis 幾乎不可分辨(該區設計矩陣病態)
               → 量 s 的最佳位置
    line1 內   天光線主導流量,basis 的能量幾乎都集中在這裡
               → 量 cₖ 的最佳位置
...
def main():
    ...
    # ---------- 兩階段 ----------
    # stage 1 用哪個區域定 s,是關鍵。line1 只遮掉最強的線,它的補集裡還藏著
    # 後面幾輪才找到的弱線 —— 那些正的流量會把 s 推高。真正無線的區域要用
    # 「最後一輪遮罩」的補集。

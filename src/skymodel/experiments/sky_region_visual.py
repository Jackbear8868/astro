...
這一支不挑界線,只把星系的光延伸到哪裡畫到眼睛能直接判讀,座標軸標好格線,由人自
己讀出要切在哪一格。三件事讓暈看得見:

① 拉伸貼著背景不貼峰值:核心遠亮於背景,用峰值定上限的話暈整片都是同一個顏色。
   上限只取背景 + 8 sigma,核心過曝換來暈的層次。
② 等光度線畫在平滑過的影像上:單像素的雜訊會讓等高線碎成一堆圈圈。先高斯平滑 2 px
   再畫,線條才代表這一片區域的亮度。四條線是背景之上 1 / 2 / 5 / 15 sigma,1 sigma
   那條就是肉眼可及的星系邊界,和 SExtractor 的遮罩一比就知道差多少。
③ sigma 由沒有源的地方量:全圖量會被星系本身撐大,等光度線就被推到太裡面。

底圖優先讀重建版 `*_pseudo_r_nosky.fits`(從 nosky cube 在 5625-6825 A 等權平均),
沒有才退回教授的原檔 —— 原檔有些沒扣天空,和其他的不在同一個基準上。
...
def prep(n, smooth, margin):
    ...
    # 視野最外圈不畫等高線:那裡曝光次數少、值偏高又偏吵,bg 與 sigma 本來就排除
    # 了它。留著會沿邊框描出一圈亮線,看起來像整個視場都超過門檻。

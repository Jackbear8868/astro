...
三顆曝光的視場形狀還不一樣 (306x315 vs 315x306),偏差量因此每顆都不同 —— 這正是
手寫 FFT 互相關把 EXP2 算錯 2 px 的原因。先扣平均、再除以重疊像素數也救不回來,
因為缺值邊界本身就是結構,會被互相關當成訊號。
...
實測結論:遮罩版 20/20 全對(誤差上限 0.5 px = 整數量化下限);未遮罩的純相位相關
(normalization='phase')有 3/20 大錯,最糟一次差 232 px,故不採用。

skimage 回傳值的方向:phase_cross_correlation(ref, mov) 給的是「要把 mov 移多少才
對得上 ref」,和本檔約定差一個負號,即 (oy, ox) = -(raw[0], raw[1])。這個負號不是
用讀文件決定的,是 --synthetic 用已知位移量出來的。

...
判讀的方式和 ID 10 那個例子一樣:一個 z 若是對的,它預言的**每一條**強線都
要在資料上找得到;只要有一條該出現卻沒出現(殘差在那裡出現一根朝下的尖峰),
那個 z 就被否定了。反過來,如果教授的 z 預言的線都在,只是深度不夠,那就是
模板的表達力不足,不是紅移錯。
...
from step4b_window_fit import STAR_WINDOW, GAL_WINDOW, make_tag, EIGEN_GAL, TPL_DIR

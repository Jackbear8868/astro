...
動機:blank 殘差裡量到了「非全場共有」的空間結構 ——
    逐行/逐列中位數的散布是純雜訊預期值的 2.5 倍
    空間相關延伸到 20 px(遠超過 4 px 的 PSF 與重採樣核)
    沿 x 的相關一直是沿 y 的 2 倍(各向異性,純雜訊不會這樣)
MUSE 的 slice 沿特定方向排列,切片級的系統性(例如 LSF 隨視場變化)正好會
產生這種形態。若真是如此,「全場共用一組 basis」本身就是問題。

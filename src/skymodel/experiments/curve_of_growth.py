...
ring_consistency.py 算的是逐圈獨立的 Δchi2,那條曲線隨表面亮度往外掉而遞減,所以
需要一個從平移空白區量出來的門檻來決定何時算掉進雜訊裡。這支改算累積的,把足跡加
第 1 到第 r 圈當成一整塊區域去擬合:

    Δchi2_cum(r) = 對 <足跡 ∪ 環1..環r> 的平均光譜做同樣的巢狀擬合

加進一圈有訊號的區域,訊號增加得比雜訊快,累積 Δchi2 上升;加進一圈只有雜訊的,
平均值被稀釋,累積 Δchi2 下降。所以曲線有一個極大值,那個峰就是再長下去只會變差
的位置 —— 和孔徑光度的 curve of growth 同一個論證,最佳孔徑在 S/N 的峰值。峰值不
需要門檻,也就不需要虛無分布。同時畫三條線對照:

    per-ring     只有第 r 圈(= ring_consistency.py 在算的)
    cum shell    第 1 到第 r 圈的聯集,不含足跡
    cum total    足跡 + 第 1 到第 r 圈的聯集
...
def main():
    ...
    for i in ids:
        ...
        # 峰值取累積殼那條:P 就是從足跡量出來的,用 P 擬合足跡必然完美,含足跡
        # 的曲線起點是循環論證,之後加任何東西都只會稀釋它。

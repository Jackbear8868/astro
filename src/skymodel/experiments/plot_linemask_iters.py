...
回答的問題:遮罩為什麼一路成長(遮掉發射線 → 連續譜下降、sigma 縮小 → 門檻下移)、
遮罩是不是逐輪累加(不是,每輪都用原始 mean_sky 重新判定)、迭代停在哪裡又為什麼停。

讀 step3 存下的 continuum_iterations.npz,輸出到
results/skymodel/evaluation/sky_basis/linemask_iters_{pNN}/,每一輪兩張:
    iter{N}_masked.png    被遮掉的通道換顏色,連同連續譜與上下門檻
    iter{N}_unmasked.png  沒被遮掉的通道 —— 連續譜就是在這些通道上擬的
檔名講的是通道被怎麼處理,不是它們「是不是線」:判準只知道某通道超出了門檻。

estimate_continuum 先判斷停止條件才存檔,觸發停止的那一輪因此不在檔裡;
--with-rejected 從最後一輪的遮罩重算它,存成 iter{N+1}_*_rejected.png。
...
# 模型(連續譜與門檻包絡)用暖色,資料與判定結果用冷色,連續譜才不會被埋在被
# 標記的通道裡。兩條門檻同色系深淺,因為它們是同一個包絡的上下緣。
C_SKY, C_CONT, C_UP, C_DN = "0.72", "#b30000", "#e6550d", "#fdae61"
C_HI,  C_LO = "#2ca02c", "#6a51a3"
C_KEEP = "#d62728"      # 沒被遮掉的通道 —— 連續譜就是在這些點上擬的

# masked 與 unmasked 要能疊著切換,兩張的幾何就必須逐像素相同:座標框釘死在這個
# 矩形(左, 下, 寬, 高,畫布比例),存檔也不裁切 —— bbox_inches="tight" 依內容裁,
# 兩張的圖例內容不同,寬度就會差一點。右邊留的空白放圖例,窄了會被畫布切掉。
...
def main():
    ...
    if args.with_rejected:
        # 先確認本檔的 WINDOW / THRESHOLDS 能重現最後一輪:對不上的話,重算出來
        # 的是另一組參數下的結果,而圖上沒有任何地方看得出這件事。
    ...
    # ---------------- 每一輪一張圖 ----------------
    # 目錄名帶上工作區名稱,否則後跑的 pointing 會無聲蓋掉前一顆的圖。
    ...
    def blank_panel(title):
        ...
        座標框用 add_axes 釘在 AXES_RECT,不交給自動版面 —— 位置只由那一個常數
        決定,不會被 rcParams 的邊界或日後的 tight_layout 挪走。
        """
        fig = plt.figure(figsize=args.figsize)
        a = fig.add_axes(AXES_RECT)
        # x 範圍明寫:add_collection 加的資料不會帶動自動縮放,而兩張圖共用這
        # 一行,波長也就一定對得上。
    ...
    def panel(i, title, name):
        ...
        detect_lines 的判準是雙向的 (ms > C + t0*sigma) | (ms < C - t1*sigma),
        所以「被判成線」拆得成上下兩側,低於 -t1*sigma 的那一半才不會被忽略。
        上色的是 mean_sky 曲線本身,不是從連續譜畫到資料值的垂直線 —— 幾百根短
        棒疊起來會在連續譜上方糊成一條實心色帶,還把連續譜自己蓋掉。
        """
        hi = ms > C[i] + THRESHOLDS[0] * S[i]
        lo = ms < C[i] - THRESHOLDS[1] * S[i]
        # 重建的遮罩和 step3 存的不同,就代表這裡的 THRESHOLDS 對不上,圖上
        # 畫的門檻是假的。
        ...
        # LineCollection 沒有可用的圖例代表,樣本自己造。上下兩色不列:綠色在
        # +sigma 之上、紫色在 -sigma 之下,圖上看得出來。
        legend(a, [
    ...
    def panel_unmasked(i, name, title):
        ...
        這張要看的是「剩下哪些通道」,多畫三條線只會蓋住那些點。未遮罩的通道
        畫成紅點疊在 mean_sky 上,點的疏密就是可用資料的分布。
        """
        keep = ~M[i]
        fig, a = blank_panel(title)
        a.plot(wl, ms, lw=LW_SKY, color=C_SKY)
        a.plot(wl[keep], ms[keep], ".", ms=4.0, color=C_KEEP, lw=0)
        # 圖例的紅點自己造,不用 markerscale:markerscale 是乘法,圖上的點一改
        # 圖例就跟著改,而這裡要的是一顆和字級相稱的點。
        legend(a, [
            Line2D([], [], color=C_SKY, lw=2.6, label="mean sky"),
    ...
    for i in range(n_iter):
        # 未採用的那輪走同一段程式、同一個標題格式,只有檔名帶 _rejected;兩套
        # 畫法會讓兩張圖之間多出一個「是不是畫法不一樣」的解釋。
